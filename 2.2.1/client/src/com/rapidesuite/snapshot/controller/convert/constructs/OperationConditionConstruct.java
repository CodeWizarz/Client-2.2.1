...
public class OperationConditionConstruct extends GenericConstruct{
	...
	public OperationConditionConstruct(ConvertEngine convertEngine) {
		super(convertEngine);
	}
		...
		if (isSuccess) {
			...
			if (ifThenElseTypeInternal!=null && ifThenElseTypeInternal.length>0) {
				...
				if (operationArray!=null && operationArray.length>0) {
					LoopRecordsTypeConstruct.processOperations(convertEngine,operationArray,sourceDataRow,variableToValueMap,fieldNameToPositionMap,sourceCode);
					return;
				}
				else {
