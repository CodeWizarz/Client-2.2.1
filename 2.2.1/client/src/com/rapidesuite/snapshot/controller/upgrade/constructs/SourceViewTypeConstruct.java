...
public class SourceViewTypeConstruct extends GenericConstruct {
	...
	public SourceViewTypeConstruct(UpgradeEngine upgradeEngine) {
		super(upgradeEngine);
	}
	...
	public void process(SourceType sourceType) throws Exception {
		...
		if (type.equals(FromType.Type.CARTESIAN) || type.equals(FromType.Type.LEFT_JOIN) || type.equals(FromType.Type.LEFT_JOIN_STRICT)) {
			...
			List<String[]> sourceLeftDataRows=upgradeEngine.getSourceCodeToDataRowsMap(leftSourceCode);
			if (sourceLeftDataRows==null){
				throw new Exception("No data found for view source code: '"+leftSourceCode+"'");
			}
			
			List<String[]> sourceRightDataRows=upgradeEngine.getSourceCodeToDataRowsMap(rightSourceCode);
			...
			Map<String, Integer> leftFieldNameToPositionMap=upgradeEngine.getFieldNameToPositionMap(leftSourceCode);
			if (leftFieldNameToPositionMap==null){
				throw new Exception("No fields found for the view source code: '"+leftSourceCode+"'");
			}
			Map<String, Integer> rightFieldNameToPositionMap=upgradeEngine.getFieldNameToPositionMap(rightSourceCode);
			...
			for (int i=startFromLeftRecordIndex;i<endFromLeftRecordIndex;i++) {
				...
				if (index % 1000==0) {
					...
					fusionInventoryRow.setRemarks("Creating View ("+Utils.formatNumberWithComma(index)+
			...
			upgradeEngine.getSourceCodeToConvertSourceTypeGenericMap().put(sourceType.getCode(), convertSourceTypeGeneric);
		}
		else {
	...
	private String[] createDataRow(String[] leftSourceDataRow,String[] rightSourceDataRow,List<ColumnTypeDetail> columnTypeDetailList) throws Exception {
		/*
		 * Must add the navigation mapper in case one of the sources is a DFF,
		 * the navigation mapper contains the concatenated field names.
		 */
		/*<c>RSC Data Label</c>
		...
		*/
		...
		/*
		String[] dataRow=new String[columnTypeDetailList.size()+4];
		int navigationMapperIndex=dataRow.length-3;

		dataRow[navigationMapperIndex]="";
		int leftNavigationMapperIndex=leftSourceDataRow.length-3;
		String leftNavigationMapper=leftSourceDataRow[leftNavigationMapperIndex];
		if (leftNavigationMapper!=null && !leftNavigationMapper.isEmpty()) {
			dataRow[navigationMapperIndex]=leftNavigationMapper;
		}
		else {
			if (rightSourceDataRow!=null) {
				int rightNavigationMapperIndex=rightSourceDataRow.length-3;
				String rightNavigationMapper=rightSourceDataRow[rightNavigationMapperIndex];
				if (rightNavigationMapper!=null && !rightNavigationMapper.isEmpty()) {
					dataRow[navigationMapperIndex]=rightNavigationMapper;
				}
			}
		}
		 */
