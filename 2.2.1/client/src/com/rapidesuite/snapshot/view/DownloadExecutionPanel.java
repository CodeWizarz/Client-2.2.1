...
public class DownloadExecutionPanel extends JPanel {

	private JLabel statusLabel;
	private JLabel totalTasksLabel;
	private JLabel totalTasksErrorsLabel;
	private JLabel totalExportedRecordCountLabel;
	...
	public DownloadExecutionPanel(GenericController genericController,TabSnapshotsPanel tabSnapshotsPanel,
			String executionTimeLabelText,String totalRecordsCountLabelText,String outputFolderLabelText ) {
		...
		createComponents(executionTimeLabelText,totalRecordsCountLabelText,outputFolderLabelText);
	}
	...
	public void createComponents(String executionTimeLabelText,String totalRecordsCountLabelText,String outputFolderLabelText){
		ImageIcon ii=null;
		URL iconURL =null;
		...
		tempPanel.setLayout(new BoxLayout(tempPanel, BoxLayout.X_AXIS));
		northPanel.add(tempPanel);
		label=new JLabel("Errors (click to open Log folder or filter the grid by Status) : ");
		InjectUtils.assignArialPlainFont(label,InjectMain.FONT_SIZE_NORMAL);
		label.setForeground(Color.white);
		tempPanel.add(label);
		...
		totalTasksErrorsLabel.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		totalTasksErrorsLabel.addMouseListener(new MouseAdapter() {
	            @Override
	            public void mouseClicked(MouseEvent e) {
	            	processActionOpenLogFolder();
	            }
	        });
		...
		tempPanel.setLayout(new BoxLayout(tempPanel, BoxLayout.X_AXIS));
		northPanel.add(tempPanel);
		label=new JLabel(totalRecordsCountLabelText);
		InjectUtils.assignArialPlainFont(label,InjectMain.FONT_SIZE_NORMAL);
		label.setForeground(Color.white);
		tempPanel.add(label);
		totalExportedRecordCountLabel=new JLabel();
		InjectUtils.assignArialPlainFont(totalExportedRecordCountLabel,InjectMain.FONT_SIZE_NORMAL);
		totalExportedRecordCountLabel.setForeground(Color.white);
		tempPanel.add(totalExportedRecordCountLabel);		
	...
	protected void processActionOpenLogFolder() {
		try {
			if (Desktop.isDesktopSupported()) {
				Desktop desktop = Desktop.getDesktop();
				desktop.open(FileUtils.getLogFolder());
			}
			else {
				SeleniumUtils.startLinuxFileBrowser(FileUtils.getLogFolder());
			}
		} 
		catch (Exception e) {
			FileUtils.printStackTrace(e);
	...
	public JLabel getTotalExportedRecordCountLabel() {
		return totalExportedRecordCountLabel;
	}
