...
public class ConvertDataGrid extends JFrame  {
	...
	public ConvertDataGrid(JFrame rootFrame,Inventory inventory,List<String[]> dataRows,boolean isSourcePanel,
	...
	private void createComponents(final boolean isSourcePanel){
		...
		for (String[] dataRow:dataRows) {
			...
			for (Field field:fields) {
				String sourceValue=dataRow[colIndex];
				if (sourceValue==null || sourceValue.isEmpty() ) {
					if (field.getMandatory() && (columnNamesToIgnore==null || !columnNamesToIgnore.contains(field.getName())) ) {
				...
				if (!substitutionKeysSet.isEmpty() && !sourceValue.isEmpty()) {
				...
				try{
					...
					if (!isSourcePanel){
						...
						if (value!=null && !value.isEmpty() ) {						
							Set<String> substitutionKeysSet=columnIndexToSubstitutionKeysSetMap.get(inventoryColIndex);
							if (!substitutionKeysSet.isEmpty() && !value.isEmpty() ) {
		...
		if (isSourcePanel) {
			//UIUtils.setColumnSize(table,COLUMN_HEADING_SOURCE_STATUS,COLUMN_HEADING_MIN_WIDTH,COLUMN_HEADING_STATUS_WIDTH,false);
		}
		else {
		...
		convertDataGridTotalsPanel.getGridRowsCountLabel().setText(UIConstants.LABEL_GRID_ROWS+dataRows.size());
		...
		if (isSourcePanel) {
			//filteringTable.getFixedTable().getColumn(COLUMN_HEADING_SOURCE_STATUS).setHeaderRenderer(new MultiLineHeaderRenderer(Color.BLACK,Color.WHITE));
		}
		else {
