...
procJvmArgumentsRetriever = subprocess.Popen([javaCmd, '-cp', cp, '-Xmx1024m', 'com.rapidesuite.client.common.util.Config', 'BUILD_JVM_ARGUMENTS'], stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
...
javaArguments += ' -DBUILD_ITERATION_LOG_FOLDER='+os.path.join(multiuser_path_setting.rapid_client_directory, 'iteration_logs')
...
cmd = javaCmd + "  " + javaArguments+" -cp "+cp+" com.rapidesuite.build.BuildMain"
